...
def execute(filters=None):
    ...
    if filters.date_wise_bifurcation:
        columns.insert(2, {"label": "Supplier Name", "fieldtype": "Data", "fieldname": "party_naming", "width": 200})
    else:
        columns.insert(1, {"label": "Supplier Name", "fieldtype": "Data", "fieldname": "party_naming", "width": 200})
    ...
    to_date = filters.get('to_date')
    if filters.date_wise_bifurcation:
        data = add_receipts_data_bifurcation(data, from_date, to_date, mode_of_payment_filter,)
    else:
        data = add_receipts_data(data, from_date, to_date, mode_of_payment_filter)
...
def add_receipts_data(data, from_date, to_date, mode_of_payment_filter):
    ...
    for row in data:
        ...
        row['cash_amount'] = cash_receipts.get(party, 0.0)
        row['bank_amount'] = bank_receipts.get(party, 0.0)
        row['party_naming'] = frappe.db.get_value("Supplier", party, "supplier_name")

    return data


def add_receipts_data_bifurcation(data, from_date, to_date, mode_of_payment_filter):
    mode_of_payment_conditions = ''
    if mode_of_payment_filter:
        placeholders = ', '.join(['%s'] * len(mode_of_payment_filter))
        mode_of_payment_conditions = f"AND mode_of_payment IN ({placeholders})"
    ...
    bank_entries = frappe.db.sql(f"""
        ...
        FROM `tabPayment Entry`
        WHERE posting_date BETWEEN %s AND %s
        AND payment_type = 'Pay' AND docstatus = 1
        AND mod_type = 'Bank'
        {mode_of_payment_conditions}
